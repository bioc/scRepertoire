...
# Note: GMM fitting can occasionally fail to converge depending on the data.
# If this occurs, a manual threshold (e.g., 0.15) can be used instead.
threshold_output <- tryCatch(
  findThreshold(dist_heavy$dist_nearest,
                progress = TRUE,
                method = "gmm",
                model = "gamma-norm",
                cutoff = "user",
...
threshold_auto <- if (!is.null(threshold_output)) threshold_output@threshold else 0.15
```
...
# Exporting scRepertoire Clones to dowser

The sections above flow from `Immcantation` into `scRepertoire`. You can also go the other direction: start in `scRepertoire` and hand full-length sequences to `dowser` for B cell lineage / phylogenetic analysis.

## Retaining full-length sequences

By default `scRepertoire` stores only the CDR3 in the `CTnt`/`CTaa` columns. `dowser` reconstructs ancestral sequences from IMGT-gapped alignments, so it needs the full-length `sequence_alignment` and `germline_alignment` fields. Those come from an `Immcantation`/AIRR annotation pipeline (IgBLAST + `MakeDb` + `CreateGermlines`), so import an annotated table and retain the alignments through the combine step.

`loadContigs()` keeps the standardized `sequence`/`sequence_aa` columns automatically (and `sequence_alignment`/`germline_alignment` when present). `combineBCR()` then carries them forward when you opt in with `retain.sequences`, as additional per-chain columns that never disturb `CTaa`/`CTnt`/`CTgene`/`CTstrict`.

```{r eval = FALSE}
# `contigs` is an Immcantation / AIRR-annotated table loaded with loadContigs()
combined <- combineBCR(contigs,
                       samples = "P1",
                       retain.sequences = c("sequence_alignment",
                                            "germline_alignment"))

# The combined object now carries per-chain full-length columns alongside CT*
head(combined[[1]])
```

## Building dowser clones

`exportDowser()` reshapes the combined object into an AIRR-compliant data frame, mapping each cell's clone definition (`CTstrict` by default) to dowser's `clone_id` and adding the `sequence_id`/`junction_length` housekeeping fields `formatClones()` expects.

```{r eval = FALSE}
airr <- exportDowser(combined, write.file = FALSE)

# Hand off directly to dowser
clones <- dowser::formatClones(airr)

# Or let scRepertoire call formatClones() for you
clones <- exportDowser(combined, run.formatClones = TRUE)
```

If the alignment/germline columns are missing (for example raw 10x Cell Ranger output, which never contains them), `exportDowser()` stops with guidance on how to obtain and retain them. From here you can build trees with `dowser::getTrees()` and visualize lineages with `dowser::plotTrees()`.
