...
test_that("`proportion` parameter works correctly", {
  table_prop <- clonalOccupy(scRep_example, x.axis = "seurat_clusters", proportion = TRUE, export.table = TRUE)
...
test_that("`facet.by` parameter works correctly", {
  ...
  table_facet <- clonalOccupy(scRep_example, x.axis = "seurat_clusters", facet.by = "Type", export.table = TRUE)
...
test_that("`na.include` parameter works correctly", {
  ...
  table_no_na <- clonalOccupy(scRep_example, x.axis = "seurat_clusters", export.table = TRUE, na.include = FALSE)
  expect_false(any(is.na(table_no_na$cloneSize)))
  
  # Test with na.include = TRUE
  table_with_na <- clonalOccupy(scRep_example, x.axis = "seurat_clusters", export.table = TRUE, na.include = TRUE)
